...
describe('Fast-Food-Fast orders test', () => {
  ...
  describe('Test endpoint to place order', () => {
    ...
    it('it should reject add food when user provides invalid address', (done) => {
      chai.request(app)
        .post('/api/v1/orders')
        .set('x-access-token', token)
        .send(order)
        .end((err, res) => {
          ...
          res.type.should.equal('application/json');
          res.body.should.have.property('status', 'error');
          res.body.should.have.property('message', 'Invalid Address');
          done();
        });
    });
    it('it should successfully place user\'s order when user provides order information', (done) => {
      order.address = '58 adams road, ikeja lagos',
      chai.request(app)
        .post('/api/v1/orders')
        .set('x-access-token', token)
        .send(order)
